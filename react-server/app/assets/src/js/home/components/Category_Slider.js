import React, { Component } from 'react';
import Img from '../../common/Img.js';
const Category_Slider = (props) => {
    let { id,index, style, count, click, hover,mouseOut } = props;
    const slider = 'data:image/png;base64,';
    let imgs = [];
    if (count > 0) {
        for (let key = 0; key < count; key++) {
            imgs.push(<Img id={key + 'Img'} key={key + 'Category_Slider'} src={slider} style={index == key ? 'hover' : ''} click={click} hover={hover} mouseOut={mouseOut} />)
        }
    }
    return (
        <div id={id} className={style}>
            {imgs}
        </div>
    );
}
export default Category_Slider;