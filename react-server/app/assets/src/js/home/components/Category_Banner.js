...
const Category_Banner = (props) => {
    let { id, style, imgList, index, hover, mouseOut } = props;
    let imgs = [];
    let key = 0;
    if (Array.isArray(imgList)) {
        for (key; key < imgList.length; key++) {
            imgs.push(<Img key={key + 'Category_Banner'} src={imgList[key]} mouseOut={mouseOut} hover={hover} style={key == index ? '' : 'display'} />);
        }
    } else {
